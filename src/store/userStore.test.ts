...
describe("useUserStore", () => {
  ...
  describe("addOrder()", () => {
    it("adds a new order to the orders list", () => {
      const order = makeOrder({ id: "order-unique", total: 25.0 });
      useUserStore.getState().addOrder(order);
      expect(useUserStore.getState().orders).toHaveLength(1);
      expect(useUserStore.getState().orders[0].total).toBe(25.0);
    });
  ...
  describe("Cart total calculation", () => {
    it("calculates correct total for multiple items", () => {
      useUserStore.getState().addToCart(makeCartItem({ id: "p1", price: 10.0, quantity: 2 }));
      useUserStore.getState().addToCart(makeCartItem({ id: "p2", price: 5.5, quantity: 1 }));
      const total = useUserStore
        .getState()
        .cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
      expect(total).toBeCloseTo(25.5, 2);
    });
